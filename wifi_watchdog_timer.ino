...
void wifi_watchdog_timer_setup(int interrupt_seconds) {
 ...
 Serial.println("wifi_watchdog_timer_setup() eseguita");
}
...
void wifi_watchdog_timer_print_info(unsigned int intervallo_ms){
  if (millis()>wifi_watchdog_timer_print_info_last_passage+intervallo_ms){
    ...
    if (WiFi.status() == WL_CONNECTED) {Serial.println("wifi_watchdog_timer_print_info(): wifi_watchdog: WiFi connected! - "+WiFi.localIP().toString());}
    if (WiFi.status() != WL_CONNECTED) Serial.println("wifi_watchdog_timer_print_info(): wifi_watchdog: Wifi NOT connected since "+String(interruptCounter)+" min");
  }
